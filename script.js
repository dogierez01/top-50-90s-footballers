...
let lifetimeScore = parseInt(localStorage.getItem('90sFootballScore')) || 0;
let completedLessons = JSON.parse(localStorage.getItem('completed90sFootballLessons')) || [];
...
const stations = [
    ...
    {file:"23_LuisSuarez.mp3", title:"Luis Suárez"},
...
function runQuiz(lesson) {
    ...
    document.getElementById('btn-speak').onclick = function() {
        ...
        window.currentRec = new (window.webkitSpeechRecognition || window.Recognition)();
...
function finishQuiz() {
    lifetimeScore += totalScore; localStorage.setItem('90sFootballScore', lifetimeScore);
    const fn = decodeURIComponent(audio.src.split('/').pop());
    if(!completedLessons.includes(fn)) {
        completedLessons.push(fn); localStorage.setItem('completed90sFootballLessons', JSON.stringify(completedLessons));
    }
